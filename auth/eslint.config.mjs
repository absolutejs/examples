...
export default defineConfig([
  ...
  },
  {
    ...
    rules: {
      ...
      "absolute/inline-style-limit": ["error", 3],
      ...
      "absolute/no-inline-object-types": ["error", { minProperties: 3 }],
  ...
  {
    files: ["absolute.config.ts", "drizzle.config.ts"],
    languageOptions: {
      parserOptions: {
        project: null,
      },
  ...
  {
    files: ["tsconfig.json", "package.json", ".prettierrc.json", "knip.json"],
    rules: {
      "@typescript-eslint/no-unused-expressions": "off",
    },
  },
  {
    // Ambient module shims (e.g. *.vue) legitimately use a default export.
    files: ["**/*.d.ts"],
    rules: {
      "no-restricted-exports": "off",
    },
